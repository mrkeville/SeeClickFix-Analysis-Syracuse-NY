...
seeclickfix_clean <- seeclickfix %>% 
  ...
  mutate(mean_days_to_closed=round(mean(days_to_closed, na.rm=T)),0)%>%
...
ggplot(data=seeclickfix_clean_avgclosed)+
  geom_col(aes(x=reorder(Category, -`Number of Requests`),y=days_to_closed, fill= Category))+
...
seeclickfix_clean_open_close<-seeclickfix_clean%>%
...
ggplot(data=seeclickfix_clean_open_close)+
  geom_col(aes(x=reorder(Category, -`Number of Requests`), y= requests_open, fill=Category))+
...
seeclickfix_clean_quad_avg<-seeclickfix_clean%>%
  filter(Quadrant!="Syracuse Boundary" & !is.na(Quadrant))%>%
  group_by(Agency_Name, Quadrant, mean_days_to_closed)%>%
  summarise(`Number of Requests`=n())%>%
  arrange(Quadrant,desc(`Number of Requests`))
...
## Number of Requests by Quadrant----
...
ggsave("requestsbyquadrant.pdf", height = 10, width=15)
...
seeclickfix_clean_outstanding <- seeclickfix_clean %>% 
  mutate(Quadrant=toupper(Quadrant))%>%
  left_join(syr_quadrants, by=c("Quadrant"="NAME"))

seeclickfix_clean_outstanding<-seeclickfix_clean_outstanding%>%
  group_by(Quadrant, Category,Agency_Name)%>%
  filter(!is.na(Quadrant))%>%
  summarize(`Number of Requests`=n(),
            requests_open=sum(is.na(Closed_at_local)))

ggplot(data=seeclickfix_clean_outstanding,
       aes(y = `requests_open`, 
           axis1 = Quadrant, 
           axis2 = Agency_Name)) + 
...
ggsave("outstanding_quad.pdf", height=10, width= 15)